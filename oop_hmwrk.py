class Student:
    ...
    def avg_grade_hw(self):  # средняя оценка за ДЗ
        ...
        return round(total_grades / total_count, 1) if total_count > 0 else "Нет оценок"
...
class Reviewer(Mentor):
    def rate_hw(self, student, course, grade):  # проверяющий оценивает ДЗ студента
        if (
...
def avg_grade_for_course(students_list, course):
    ...
    for student in students_list:
        ...
        if (
            course in student.finished_courses and course not in student.grades
        ):  # все равно не учитываются оценки за завершенные курсы (Git), причем и у лекторов, не могу найти причину
